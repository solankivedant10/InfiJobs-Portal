...
import { Home, Menu, ChevronLeft, ChevronRight, Coffee } from 'lucide-react';
import type { LucideIcon } from 'lucide-react';
import {
    Code, Layers, ArrowRight, GitBranch, FileCode, Database, Leaf, Globe, Zap, CheckCircle
} from 'lucide-react';
import { javaFullStackTutorialData } from '../../data/javaFullStackTutorialData';
import CodeBlock from '../ml-portal/components/CodeBlock';

const iconMap: Record<string, LucideIcon> = {
    Coffee, Code, Layers, ArrowRight, GitBranch, FileCode, Database, Leaf, Globe, Zap, CheckCircle
};

const JavaSidebar: React.FC<{ isCollapsed: boolean; onToggle: () => void }> = ({ isCollapsed, onToggle }) => (
    <aside className={`fixed left-0 top-0 h-screen bg-[#0f1419] border-r border-gray-800/50 flex flex-col z-40 transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-56'}`}>
        <div className="p-4 border-b border-gray-800/50 flex items-center justify-between">
            <div className={`flex items-center gap-3 ${isCollapsed ? 'justify-center w-full' : ''}`}>
                ...
                {!isCollapsed && <span className="font-semibold text-gray-100">Java Portal</span>}
            </div>
            {!isCollapsed && (
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-1">
            {javaFullStackTutorialData.map((topic) => {
                const Icon = iconMap[topic.icon] || Coffee;
                return (
                    <NavLink
                        key={topic.id}
                        to={`/java-portal/${topic.id}`}
                        className={({ isActive }) => `flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all ${isActive ? 'bg-red-500/20 text-red-400' : 'text-gray-400 hover:bg-gray-800/50'} ${isCollapsed ? 'justify-center' : ''}`}
...
const JavaTopicView: React.FC = () => {
    const { topicId } = useParams<{ topicId: string }>();
    const location = useLocation();
    const isIndex = location.pathname === '/java-portal' || location.pathname === '/java-portal/';

    if (isIndex) {
        return (
            <div className="text-center py-12 px-4">
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-red-500/20 to-orange-500/20 border border-red-500/30 mb-8">
                    <Coffee className="w-4 h-4 text-red-400" />
                    <span className="text-sm font-medium text-red-400">FULL STACK DEVELOPMENT</span>
                </div>
                <h1 className="text-4xl md:text-5xl font-bold mb-6">
                    <span className="text-gray-100">Java Full Stack </span>
                    <span className="bg-gradient-to-r from-red-400 to-orange-400 bg-clip-text text-transparent">Tutorial</span>
                </h1>
                <p className="text-gray-400 text-lg max-w-2xl mx-auto mb-8">
                    Master Java from core concepts to Spring Boot. Build enterprise applications with REST APIs, Hibernate, and modern frameworks.
                </p>
                <Link to="/java-portal/intro-java-fullstack" className="inline-flex items-center gap-2 text-red-400 hover:text-red-300">
    ...
    const topic = javaFullStackTutorialData.find(t => t.id === topicId);
    ...
    return (
        <div className="px-4 md:px-8 py-8 max-w-4xl mx-auto">
            <div className="relative rounded-2xl overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-red-500/20 via-orange-500/20 to-amber-500/20 rounded-2xl" />
...
const JavaPortalLayout: React.FC = () => {
    const [collapsed, setCollapsed] = useState(false);
    const [mobileOpen, setMobileOpen] = useState(false);

    return (
        <div className="min-h-screen bg-[#0a0d12]">
            <div className="hidden md:block"><JavaSidebar isCollapsed={collapsed} onToggle={() => setCollapsed(!collapsed)} /></div>
            ...
            <div className={`fixed left-0 top-0 h-screen z-50 md:hidden transform transition-transform ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <JavaSidebar isCollapsed={false} onToggle={() => setMobileOpen(false)} />
            </div>
            <div className={`transition-all duration-300 ${collapsed ? 'md:ml-16' : 'md:ml-56'}`}>
                <header className="sticky top-0 z-30 md:hidden bg-[#0f1419]/95 backdrop-blur-lg border-b border-gray-800/50">
                    <div className="flex items-center justify-between px-4 py-3">
                        ...
                        <span className="font-semibold text-gray-100">Java Portal</span>
                ...
                <header className="hidden md:block sticky top-0 z-30 bg-[#0a0d12]/95 backdrop-blur-lg border-b border-gray-800/50">
                    <div className="flex items-center justify-between px-6 py-3">
                        ...
                        <div className="hidden lg:block w-48 h-1.5 rounded-full bg-gradient-to-r from-red-500 to-orange-500" />
                        <div className="w-24" />
                    </div>
                </header>
...
export { JavaPortalLayout, JavaTopicView };
export default JavaPortalLayout;
