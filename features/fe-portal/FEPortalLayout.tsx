...
import { Home, Menu, ChevronLeft, ChevronRight, Layout } from 'lucide-react';
import type { LucideIcon } from 'lucide-react';
import {
    FileCode, Palette, Terminal, Boxes, Smartphone, Shield, FolderCode, GraduationCap
} from 'lucide-react';
import { frontendTutorialData } from '../../data/frontendTutorialData';
import CodeBlock from '../ml-portal/components/CodeBlock';

const iconMap: Record<string, LucideIcon> = {
    Layout, FileCode, Palette, Terminal, Boxes, Smartphone, Shield, FolderCode, GraduationCap
};

const FeSidebar: React.FC<{ isCollapsed: boolean; onToggle: () => void }> = ({ isCollapsed, onToggle }) => (
    <aside className={`fixed left-0 top-0 h-screen bg-[#0f1419] border-r border-gray-800/50 flex flex-col z-40 transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-56'}`}>
        <div className="p-4 border-b border-gray-800/50 flex items-center justify-between">
            <div className={`flex items-center gap-3 ${isCollapsed ? 'justify-center w-full' : ''}`}>
                ...
                {!isCollapsed && <span className="font-semibold text-gray-100">FE Portal</span>}
            </div>
            {!isCollapsed && (
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-1">
            {frontendTutorialData.map((topic) => {
                const Icon = iconMap[topic.icon] || Layout;
                return (
                    <NavLink
                        key={topic.id}
                        to={`/fe-portal/${topic.id}`}
                        className={({ isActive }) => `flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all ${isActive ? 'bg-pink-500/20 text-pink-400' : 'text-gray-400 hover:bg-gray-800/50'} ${isCollapsed ? 'justify-center' : ''}`}
...
const FeTopicView: React.FC = () => {
    const { topicId } = useParams<{ topicId: string }>();
    const location = useLocation();
    const isIndex = location.pathname === '/fe-portal' || location.pathname === '/fe-portal/';

    if (isIndex) {
        return (
            <div className="text-center py-12 px-4">
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-pink-500/20 to-rose-500/20 border border-pink-500/30 mb-8">
                    <Layout className="w-4 h-4 text-pink-400" />
                    <span className="text-sm font-medium text-pink-400">WEB DEVELOPMENT</span>
                </div>
                <h1 className="text-4xl md:text-5xl font-bold mb-6">
                    <span className="text-gray-100">Frontend Developer </span>
                    <span className="bg-gradient-to-r from-pink-400 to-rose-400 bg-clip-text text-transparent">Tutorial</span>
                </h1>
                <p className="text-gray-400 text-lg max-w-2xl mx-auto mb-8">
                    Master HTML, CSS, JavaScript, and modern frameworks. Build beautiful, responsive, and interactive web applications.
                </p>
                <Link to="/fe-portal/intro-frontend" className="inline-flex items-center gap-2 text-pink-400 hover:text-pink-300">
    ...
    const topic = frontendTutorialData.find(t => t.id === topicId);
    ...
    return (
        <div className="px-4 md:px-8 py-8 max-w-4xl mx-auto">
            <div className="relative rounded-2xl overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-pink-500/20 via-rose-500/20 to-orange-500/20 rounded-2xl" />
...
const FEPortalLayout: React.FC = () => {
    const [collapsed, setCollapsed] = useState(false);
    const [mobileOpen, setMobileOpen] = useState(false);

    return (
        <div className="min-h-screen bg-[#0a0d12]">
            <div className="hidden md:block"><FeSidebar isCollapsed={collapsed} onToggle={() => setCollapsed(!collapsed)} /></div>
            ...
            <div className={`fixed left-0 top-0 h-screen z-50 md:hidden transform transition-transform ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <FeSidebar isCollapsed={false} onToggle={() => setMobileOpen(false)} />
            </div>
            <div className={`transition-all duration-300 ${collapsed ? 'md:ml-16' : 'md:ml-56'}`}>
                <header className="sticky top-0 z-30 md:hidden bg-[#0f1419]/95 backdrop-blur-lg border-b border-gray-800/50">
                    <div className="flex items-center justify-between px-4 py-3">
                        ...
                        <span className="font-semibold text-gray-100">FE Portal</span>
                ...
                <header className="hidden md:block sticky top-0 z-30 bg-[#0a0d12]/95 backdrop-blur-lg border-b border-gray-800/50">
                    <div className="flex items-center justify-between px-6 py-3">
                        ...
                        <div className="hidden lg:block w-48 h-1.5 rounded-full bg-gradient-to-r from-pink-500 to-rose-500" />
                        <div className="w-24" />
                    </div>
                </header>
...
export { FEPortalLayout, FeTopicView };
export default FEPortalLayout;
